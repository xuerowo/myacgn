...
def process_novel_directory(novel_dir):
    try:
        ...
        # 讀取封面圖片URL
        cover_url = f"https://raw.githubusercontent.com/xuerowo/myacg/main/輕小說翻譯/{os.path.basename(novel_dir)}/cover.jpg"
        
        # 獲取小說目錄的最後修改時間
        novel_last_modified = datetime.fromtimestamp(os.path.getmtime(novel_dir))
        novel_last_modified_str = novel_last_modified.strftime('%Y-%m-%d %H:%M:%S')
        ...
        for file in sorted(files):
            ...
            # 構建章節URL
            url = f"https://raw.githubusercontent.com/xuerowo/myacg/main/輕小說翻譯/{os.path.basename(novel_dir)}/{file}"
